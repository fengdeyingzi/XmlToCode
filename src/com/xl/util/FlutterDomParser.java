...
//				printfNodes(root);
				...
				if(className.equals("LinearLayout")){
					flutter_widget = new LinearLayout();
					String orientation = root.getAttribute("android:orientation");
					flutter_widget.addParamItem("android:orientation",orientation);
				}
				...
				flutter_widget.addParamItem("android:layout_width", layout_width);
				flutter_widget.addParamItem("android:layout_height", layout_height);
				
				
				printFlutterCode(className, layout_root, root);
				return;
			}
...
//								第三个控件的宽度为 0+(2/(1+2+2))*0=0
	...
	// 输出 ios swift 代码
		void printFlutterCode(String className, String name, Node node) {
			NodeList nodelist = node.getChildNodes();
			if (node.getNodeType() == 1) {
				// 输出代码
				Element element = (Element) node;
...
//							System.out.println("    " + layout_name + ".setId(" + value + ");");
				...
				for (int n = 0; n < nodelist.getLength(); n++) {
					...
					// + nodeitem.getNodeType() + " ");
					printFlutterCode(element.getNodeName(), layout_name, nodeitem);
				}
			}
